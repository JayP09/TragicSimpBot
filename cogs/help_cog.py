    ...
    async def help(self, message, help_type="None"):
        ...
        if message.channel.name in channels:
            if help_type.upper() == 'NONE':
                ...
                embed.add_field(name='<:facts:814823810263547924> Random Facts', value="`pls fact` ")
                embed.add_field(name='<:random_number:814836293736595476> Random Number fact',
                                value="`pls fact number` ")
                embed.add_field(name='<:number:814844517508317246> Number fact', value='''`pls fact number`
                                                                                        example:`pls fact 100`''')
                embed.add_field(name='<:date_fact:814836293582061618> Date fact', value='''`pls fact date`\n at place of date input any date in ""mm/dd"" format.
                                                                                        \n Example:`pls fact 06/09` ''',inline=False)
                embed.add_field(name=' <:thinking_gorilla:814818580813578261> Animals',
                                value="`pls fact animal` ")
                embed.add_field(name='<:doge:814824520916533249> Dog fact', value="`pls fact dog` ")
                embed.add_field(name='<:cat_fact:814818758421512203> Cat fact', value="`pls fact cat` ")
                embed.add_field(name='<:panda:814836293657034773> Panda fact',
                                value="`pls fact panda` ")
                embed.add_field(name='<:fox_fact:814836293409964033> Fox fact', value="`pls fact fox` ")
                embed.add_field(name='<:bird_fact:814836293343117323> Bird fact',
                                value="`pls fact bird` ")
                embed.add_field(name='<:koala_fact:814836293648646154> Koala fact',
                                value="`pls fact koala` ")
                await message.channel.send(embed=embed)


            elif help_type.upper() == 'MEME':
                embed = discord.Embed(title="<:meme:814824521324036096> Meme Commands", colour=colour_generator())
                embed.add_field(name="<:dankmeme:814837484734382110>meme", value="`pls meme`")
                embed.add_field(name='<:dankmeme:814837484734382110>meme command', value="`meme pagename` \n return hot,new meme from any reddit page u desire.",inline=False)
                embed.add_field(name='<:dankmeme:814837484734382110>Meme from specific page',
                                value="`pls meme pagename` \n input pagename from memepage list given below "
                ...
                embed.add_field(name="<:meme:814824521324036096> Joke", value="`pls joke` ")
